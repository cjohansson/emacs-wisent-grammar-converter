...
(should (equal "(random-random-statement)" (emacs-wisent-grammar-converter--reformat-logic-block "	random-statement();  	\n\n" "random-")))

;; White-space
(should (equal "random-statement" (emacs-wisent-grammar-converter--reformat-logic-block "	random-statement;  	\n\n")))

;; Return a argument
(should (equal "$3" (emacs-wisent-grammar-converter--reformat-logic-block "  $$ = $3;  	\n\n")))

;; Function calls with arguments
(should (equal "(zend_ast_create ZEND_AST_EMPTY $3)" (emacs-wisent-grammar-converter--reformat-logic-block "  zend_ast_create(ZEND_AST_EMPTY, $3);  	\n\n")))

;; Function calls without arguments
(should (equal "(zend_ast_create)" (emacs-wisent-grammar-converter--reformat-logic-block "  zend_ast_create();  	\n\n")))

;; NULL values like    ($$ = NULLABLE)
(should (equal "$$ = nil" (emacs-wisent-grammar-converter--reformat-logic-block "  $$ = NULL;  	\n\n")))

;; Attribute assignments like    $$->attr = ZEND_NAME_NOT_FQ;
(should (equal "(put $$ 'attr 'ZEND_NAME_NOT_FQ)" (emacs-wisent-grammar-converter--reformat-logic-block "  $$->attr = ZEND_NAME_NOT_FQ;  	\n\n")))
...
;; Logical or like    $1 | $2
(should (equal "(logior $1 $2)" (emacs-wisent-grammar-converter--reformat-logic-block "  $1 | $2  	\n\n")))
...
(should (equal ";; allow single trailing comma\n(zend_ast_list_rtrim $1)" (emacs-wisent-grammar-converter--reformat-logic-block "/* allow single trailing comma */ (zend_ast_list_rtrim $1)")))
